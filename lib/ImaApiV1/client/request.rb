  ...
  module Client
    class Request
      ...
      def initialize(http_method, path, attributes={})
        attributes[:format] ||= Client.configuration.format
        attributes[:params] ||= {}

        # Set default headers
        default_headers = {
          'Content-Type' => "application/#{attributes[:format].downcase}",
          'User-Agent' => Client.configuration.user_agent
        }
        ...
        if Client.authenticated?
          attributes[:headers].merge!({:auth_token => Client.configuration.auth_token})
        end
      ...
      def get_api_key_with_prefix(param_name)
        if Client.configuration.api_key_prefix[param_name].present?
          "#{Client.configuration.api_key_prefix[param_name]} #{Client.configuration.api_key[param_name]}"
        else
          Client.configuration.api_key[param_name]
        end
      end
      ...
      def url(options = {})
        u = Addressable::URI.new(
          :scheme => Client.configuration.scheme,
      ...
      def interpreted_path
        ...
        if Client.configuration.inject_format
        ...
        if Client.configuration.force_ending_format
          unless ['.json', '.xml', '{format}'].any? {|s| p.downcase.include? s }
            p = "#{p}.#{format}"
          end
        end
        ...
        URI.encode [Client.configuration.base_path, p].join("/").gsub(/\/+/, '/')
      end
      ...
      def query_string
        ...
        self.params.each_pair do |key, value|
          ...
          if Client.configuration.camelize_params
      ...
      def make
        ...
        request_options = {
          :ssl_verifypeer => Client.configuration.verify_ssl,
